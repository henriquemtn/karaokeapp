    ...
    useEffect(() => {
        const unsubscribe = auth.onAuthStateChanged((user) => {
            ...
            if (user) {
                ...
                const q = query(housesRef, where("uid", "==", userUid));
                ...
                getDocs(q)
                    .then((querySnapshot) => {
                        ...
                        if (!querySnapshot.empty) {
                            ...
                            console.log("House information found:", docData);
                            setHouseInfo(docData);
                        } else {
                            console.log("No house information found");
                        }
                    })
                    .catch((error) => {
                    ...
                    {houseInfo && (
                        <div className="flex items-center gap-3">
                            <img
                                ...
                                src={`${houseInfo.housePic}`}
                                alt="AvatarUser"
                            />
                            <h1 className="text-xl font-medium text-gray-700 capitalize dark:text-white">{houseInfo.houseName}</h1>
                        </div>
                    )}
                </div>
            </div>
